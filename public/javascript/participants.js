...
$(document).ready(function () {
  ...
  $("#list-of-all").on("click", function () {
    fetch("/participants/", {
      ...
      .then((data) => {
        ...
        //create the table
        $("#user-table thead").append(
          `<tr><th>Full Name</th><th>Dob</th><th>Company Name</th><th>Salary</th><th>Currency</th><th>Location</th><th>Email</th></tr>`
        );
        for (let i = 0; i < arrayLength; i++) {
          const fullName =
            data.data[i].personalInfo.firstname +
            " " +
            data.data[i].personalInfo.lastname;
          ...
          const location =
            data.data[i].home.city + ", " + data.data[i].home.country;
          const email = data.data[i].email;

          $("#user-table tbody").append(
            `<tr><td>${fullName}</td><td>${dob}</td><td>${company}</td><td>${salary}</td><td>${currency}</td><td>${location}</td><td>${email}</td></tr>`
          );
        }
      })
      .catch((error) => {
  ...
  $("#personal-details-of-all").on("click", function () {
    fetch("/participants/details", {
      ...
      .then((data) => {
        ...
        //create the table
        $("#user-table thead").append(
          `<tr><th>Full Name</th><th>Dob</th></tr>`
        );
        for (let i = 0; i < arrayLength; i++) {
          ...
          $("#user-table tbody").append(
            `<tr><td>${fullName}</td><td>${dob}</td></tr>`
          );
        }
      })
      .catch((error) => {
  ...
  $("#a-person-work-details").on("click", function (e) {
    ...
    fetch(`/participants/work/${email}`, {
      ...
      .then((data) => {
        ...
        //create the table
        $("#user-table thead").append(
          `<tr><th>Company Name</th><th>Salary</th><th>Currency</th></tr>`
        );
        ...
        $("#user-table tbody").append(
          `<tr><td>${companyname}</td><td>${salary}</td><td>${currency}</td></tr>`
        );
      })
      .catch((error) => {
        console.error("Error:", error);
  ...
  $("#a-person-details").on("click", function (e) {
    ...
    fetch(`/participants/details/${email}`, {
      ...
      .then((data) => {
        ...
        //create the table
        $("#user-table thead").append(
          `<tr><th>Email</th><th>fullName</th><th>DOB</th></tr>`
        );

        const fullName = data.data.firstname + " " + data.data.lastname;
        const dob = data.data.dob;
        $("#user-table tbody").append(
          `<tr><td>${email}</td><td>${fullName}</td><td>${dob}</td></tr>`
        );
      })
      .catch((error) => {
        console.error("Error:", error);
  ...
  $("#delete-button").on("click", function (e) {
    ...
    let confirmed = confirm(
      `Are you sure you want to delete  participant with email: ${email}?`
    );
    ...
    if (!confirmed) {
      ...
      fetch(`/participants/${email}`, {
        ...
        .catch((error) => {
          console.error("Error:", error);
          alert(
            "An error occurred while deleting the participant. Please try again later."
          );
        });
    }
  ...
  $("#send-to-form").on("click", function (e) {
    ...
    $("#update-button").on("click", function (e) {
      ...
      console.log(email);
      let confirmed = confirm(
        `Are you sure you want to update  participant with email: ${email}?`
      );
      if (!confirmed) {
        return;
      } else {
        console.log(`Updating participant with email: ${email}`);
      }
    });
  });
});
